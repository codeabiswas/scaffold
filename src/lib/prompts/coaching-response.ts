...
/**
 * Generic coaching response prompt for Treatment A (no personality calibration).
 */
export function buildGenericCoachingResponsePrompt(params: {
  habitGoal: string;
  habitDescription: string;
  missedReason: "schedule_conflict" | "no_motivation" | "forgot";
  consecutiveMisses: number;
  ...
  const { habitGoal, habitDescription, missedReason, consecutiveMisses, currentSrbai } = params;
  ...
  const urgency =
    consecutiveMisses >= 3
      ? "CRITICAL: This is the 3rd+ consecutive miss. Suggest a smaller version, different time, or modified routine."
      : consecutiveMisses === 2
        ? "WARNING: Second consecutive miss. Heighten encouragement and gently explore if the current plan needs adjustment."
        : "First miss. Be supportive and address the specific barrier.";
